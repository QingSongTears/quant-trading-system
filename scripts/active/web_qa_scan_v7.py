"""
Web QA scan — 33 页面 GET + 截图 + console error + network 404 (Web QA v7)

任务 (Web QA v7 — 2026-07-01):
- v5 baseline: 33/33 通过 (100%) — T1-T12 全部生效
- v6 复跑: 探测回归 (任何从 200 → 4xx/5xx 或新增 console error 都算 regression)
- 每页截图到 output/web_qa_v7/<page>.png
...
OUTPUT_DIR = PROJECT_ROOT / "output" / "web_qa_v7"
...
def main():
    api_key = get_api_key()
    print(f"[QA v7] API key: {api_key[:8]}...")
    results = []
    ...
    print(f"\n[QA v7] {ok}/{total} passed ({ok*100//total}%)")
    print(f"[QA v7] Results: {json_path}")
    return results
